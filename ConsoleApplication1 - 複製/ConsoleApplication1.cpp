...
DWORD WINAPI EvtSubscribeCallback(EVT_SUBSCRIBE_NOTIFY_ACTION action, PVOID context, EVT_HANDLE hEvent) {
    if (action == EvtSubscribeActionDeliver) {
        ...
        swprintf_s(&insertQuery[0], querySize,
                   ...
                   L"([關鍵字], [日期和時間], [事件識別碼], [來源], [工作類別], [內容]) "
                   L"VALUES (N'%s', '%s', %d, N'%s', N'%s', N'%s')",
...
int main() {
    ...
    //                    .connect(L"DESKTOP-SO1AP2J", L"sa", L"Ww920626@")
    //                    .set_database(L"event");
    //
    //auto tb = db.command(L"select * from even with (nolock)");
    //auto name = tb[0][L"內容"].to_string();

    DatabaseAccess* DA = new DatabaseAccess();

    //std::wstring connectionStr = L"DRIVER={SQL Server};SERVER=DESKTOP-SO1AP2J;DATABASE=event;UID=sa;PWD=Ww920626@;";
    //DA->connect(connectionStr);
    DA->connect(L"DESKTOP-SO1AP2J", L"sa", L"Ww920626@");
    DA->set_database(L"event");

    SaoFU::DataTable tb = DA->command(L"select * from even with (nolock)");

    //auto name = tb[0][L"內容"].to_string();
    //fwprintf(stdout, L"內容=%ws", name.c_str());
